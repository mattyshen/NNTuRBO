        ...
        while self.n_evals < self.max_evals:
            ...
            for i in range(self.n_trust_regions):
                ...
                X_cand[i], y_cand[i], self.hypers[i] = self._create_candidates(
                    X, fX, length=self.length[i], k=self.k[i], n_training_steps=n_training_steps, hypers=self.hypers[i]
                )
            ...
            for i in range(self.n_trust_regions):
                if self.k[i] < self.k_min: #self.length[i] < self.length_min:  # Restart trust region if converged
                    idx_i = self._idx[:, 0] == i
