...
class NNTurbo1:
    ...
    Example usage:
        nnturbo1 = NNTurbo1(f=f, lb=lb, ub=ub, n_init=n_init, max_evals=max_evals)
        nnturbo1.optimize()  # Run optimization
        X, fX = nnturbo1.X, nnturbo1.fX  # Evaluated points
    """
    ...
    def __init__(
        ...
        self.k_min = self.batch_size
        self.k_max = self.n_cand #int(self.prop*self.n_cand)
        self.k_init = int(self.k_min*(self.prop**(np.log(self.k_max/self.k_min)/(2*np.log(self.prop))))) #int(self.n_cand / self.prop) #min(100 * self.dim, 5000)
    ...
    def _adjust_length(self, fX_next):
        ...
        if self.succcount == self.succtol:  # Expand trust region
            ...
            self.k = min([int(self.prop*self.k), self.k_max])
            self.succcount = 0
    ...
    def _create_candidates(self, X, fX, length, k, n_training_steps, hypers):
        ...
        #TODO: should length / 2.0 really be length / self.prop?
        lb = np.clip(x_center - weights * length / self.prop, 0.0, 1.0)
        ub = np.clip(x_center + weights * length / self.prop, 0.0, 1.0)
        ...
        return X_cand[indices[0], :], y_cand, hypers
        ...
        while self.n_evals < self.max_evals:
            ...
            while self.n_evals < self.max_evals and self.k >= self.k_min: #  and self.length >= self.length_min:
                ...
                X_cand, y_cand, _ = self._create_candidates(
                    X, fX, length=self.length, k=self.k, n_training_steps=self.n_training_steps, hypers={}
                )
